...
const EventPage = ({ pageData, globalData, preview, lang }) => {
  return <Event pageData={pageData} globalData={globalData} preview={preview} lang={lang} />
}
...
export async function getStaticProps({ params, preview = null }) {
  try {
    ...
    // Get the main event page (or first one if multiple)
    const pageDataRaw = await cmsClient.fetch(`*[_type == "event"][0]{
      _id,
      _type,
      title {
        en,
