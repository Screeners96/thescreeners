...
const PageRessourcen = ({ pageData, globalData, preview, lang }) => {

  return <Tabs pageData={pageData} filtering={false} lang={lang} />
}

export default PageRessourcen
...
export async function getStaticProps({ params, preview = null }) {
  ...
  try {
    ...
    const pageData = {
      ...
      items: (pageDataRaw?.ressourcen || []).map((item) => {
        ...
        return {
          ...item,
          _id: item._id || `ressource-${Math.random().toString(36).substr(2, 9)}`,
